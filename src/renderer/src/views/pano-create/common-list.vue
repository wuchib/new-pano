    ...
    <div class="w-full h-full flex flex-col">
        ...
        <section class="p-[8px] pt-[16px] flex-1 flex flex-col overflow-auto">
            <template v-if="!isHsAddEdit">
                ...
                <n-button class="flex-1 mr-[8px]" strong secondary @click="$emit('enterAdd')">{{ addBtnTxt }}</n-button>
                <n-button :disabled="list.length === 0" class="flex-1" strong secondary
                    @click="$emit('handleBatchDel')">{{
                        isShowCheckBoxs ? '取消' : '批量删除'
                    }}</n-button>
            </section>
            <!-- 热点列表 -->
            <section
                ...
                <n-button v-show="isShowCheckBoxs"
                    class="w-[90%] position-absolute bottom-[8px] left-[50%] translate-x-[-50%]"
                    @click="$emit('bacthDel')">确认删除</n-button>
            </section>
            </template>
